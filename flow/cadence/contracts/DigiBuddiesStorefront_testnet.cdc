...
import Helmet from 0x5246dbfcadae93b9
...
pub contract DigiBuddiesStorefront {
    ...
    pub resource Listing: ListingPublic {
        ...
        pub fun purchase(payment: @FungibleToken.Vault, collection: &NonFungibleToken.Collection{NonFungibleToken.Receiver}) {
            ...
            //Check if the account has a Helmet collection. 
            //If so, check if there are Driverz in it. 
            //If account do not have driverz, the discount will not be applied.
            if(getAccount(buyerAddress).getCapability<&{NonFungibleToken.CollectionPublic}>(Helmet.CollectionPublicPath).borrow() != nil){
                let driverzCollection:&{NonFungibleToken.CollectionPublic} = getAccount(buyerAddress).getCapability<&{NonFungibleToken.CollectionPublic}>(Helmet.CollectionPublicPath).borrow()!
